...
#define GET_VAR(_type) \
        static int32_t _get_var_##_type(const_arg_list_t &ao_list, \
                                        uint32_t &ao_count, \
                                        const waflz_pb::sec_rule_t_variable_t &a_var, \
                                        rqst_ctx *a_ctx)
namespace ns_waflz {
//: ----------------------------------------------------------------------------
//: ****************************************************************************
                        ...
                        bool a_is_key)
{
        ...
        // get negated fields
        // -------------------------------------------------
        typedef const ::waflz_pb::sec_rule_t_variable_t_match_t _m_t;
        typedef std::list <const _m_t *> _m_list_t;
        _m_list_t l_ng_list;
        for(int32_t i_m = 0; i_m < a_var.match_size(); ++i_m)
        {
                const ::waflz_pb::sec_rule_t_variable_t_match_t &l_match = a_var.match(i_m);
                    ...
                    ++i_k)
                {
                        ...
                        if(l_ng_list.size())
                        {
                                for(_m_list_t::const_iterator i_ng_m = l_ng_list.begin();
                                    ...
                                    ++i_ng_m)
                                {
                                        const ::waflz_pb::sec_rule_t_variable_t_match_t &l_ng_m = **i_ng_m;
                                        if(!l_ng_m.has_value())
                                        {
                                                continue;
                                        }
...
#define _ADD_VAR(_str, _from) do { \
        const_arg_t l_data; \
        l_data.m_key = "REMOTE_ADDR"; \
        l_data.m_key_len = sizeof("REMOTE_ADDR"); \
...
GET_VAR(MATCHED_VAR)
{
        ...
        // last matched var...
        const_arg_t l_data;
        l_data.m_key = "MATCHED_VAR";
        l_data.m_key_len = sizeof("MATCHED_VAR");
...
#define INIT_GET_VAR(_type) \
        s_var_cb_vector[waflz_pb::sec_rule_t_variable_t_type_t_##_type] = _get_var_##_type
//: ----------------------------------------------------------------------------
//: types
//: ----------------------------------------------------------------------------
...
//: \param:   TODO
//: ----------------------------------------------------------------------------
get_var_t get_var_cb(waflz_pb::sec_rule_t_variable_t_type_t a_type)
{
