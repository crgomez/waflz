                              ...
                              const pcre_list_t &a_pcre_list)
{
        ...
        // ---------------------------------
        for(arg_list_t::const_iterator i_a = ao_arg_list.begin();
            i_a != ao_arg_list.end();)
        {
                bool l_m = false;
                                    ...
                                    const pcre_list_t &a_pcre_list)
{
        ...
        // strip ignored cookies
        // ---------------------------------
        for(const_arg_list_t::const_iterator i_a = ao_arg_list.begin();
            i_a != ao_arg_list.end();)
        {
                bool l_m = false;
                l_m = key_in_ignore_list(a_pcre_list,
                               ...
                               void *a_ctx)
{
        ...
        // calculate body size
        // -------------------------------------------------
        m_body_len = l_cl > m_body_len_max ? m_body_len_max : l_cl;
        // -------------------------------------------------
        // TODO -413 on > max???
        // -------------------------------------------------
        ...
        // -------------------------------------------------
        m_body_data = (char *)malloc(sizeof(char)*m_body_len);
        bool l_is_eos = false;
        ...
        // -------------------------------------------------
        while(!l_is_eos &&
              (l_rd_count_total < m_body_len))
        {
                l_rd_count = 0;
                char *l_buf = m_body_data+l_rd_count_total;
                uint64_t l_to_read = m_body_len-l_rd_count_total;
