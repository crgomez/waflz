...
int32_t config_parser::add_rule(waflz_pb::sec_config_t &ao_config,
                                const kv_list_t &a_variable_list,
...
#define ELIF_DIRECTIVE(_a_dir) else if(strcasecmp(l_directive.c_str(), _a_dir) == 0)
        IF_DIRECTIVE("secrule")
        {
                ...
                int l_status;
                l_status = add_secrule(ao_config, l_line, a_line_len - i_char);
                if(l_status != WAFLZ_STATUS_OK)
                {
        ...
        ELIF_DIRECTIVE("secdefaultaction")
        {
                ...
                int l_status;
                l_status = add_secaction(ao_config, l_line, a_line_len - i_char, true);
                if(l_status != WAFLZ_STATUS_OK)
                {
        ...
        ELIF_DIRECTIVE("secaction")
        {
                ...
                int l_status;
                l_status = add_secaction(ao_config, l_line, a_line_len - i_char, false);
                if(l_status != WAFLZ_STATUS_OK)
                {
                        NDBG_PRINT("performing add_secaction: %s\n", l_line);
                        return WAFLZ_STATUS_ERROR;
...
#define GET_STRS(_error_msg) \
        ...
        ELIF_DIRECTIVE("secruleupdatetargetbyid")
        {
                ...
                ao_config.add_rule_remove_by_id(l_rule_id);
        }
#endif
                                 ...
                                 uint32_t a_cur_line_num)
{
        ...
        int32_t l_status;
        l_status = read_wholeline(ao_config, ao_cur_line.c_str(), ao_cur_line.length());
        if(l_status != WAFLZ_STATUS_OK)
        {
                return WAFLZ_STATUS_ERROR;
        }
                                             ...
                                             const waflz_pb::sec_config_t &a_config)
{
        ...
        if(i_sec_rule_rem_by_id < a_config.rule_remove_by_id_size())
        {
                ao_str += "SecRuleRemoveById ";
                ao_str += std::to_string(a_config.rule_remove_by_id(i_sec_rule_rem_by_id));
                ao_str += "\n";
        ...
        if(a_config.has_request_body_limit())
        {
                ...
                ao_str += std::to_string(a_config.request_body_limit());
                ao_str += "\n";
        }
        // -------------------------------------------------
        //
        ...
        if(a_config.has_request_body_no_files_limit())
        {
                ...
                ao_str += std::to_string(a_config.request_body_no_files_limit());
                ao_str += "\n";
        }
        // -------------------------------------------------
        //
        ...
        if(a_config.has_request_body_in_memory_limit())
        {
                ...
                ao_str += std::to_string(a_config.request_body_in_memory_limit());
                ao_str += "\n";
        }
        // -------------------------------------------------
        //
        ...
        if(a_config.has_pcre_match_limit())
        {
                ...
                ao_str += std::to_string(a_config.pcre_match_limit());
                ao_str += "\n";
        }
        // -------------------------------------------------
        //
        ...
        if(a_config.has_pcre_match_limit_recursion())
        {
                ...
                ao_str += std::to_string(a_config.pcre_match_limit_recursion());
                ao_str += "\n";
        }
        // -------------------------------------------------
        //
        ...
        if(a_config.has_response_body_limit())
        {
                ...
                ao_str += std::to_string(a_config.response_body_limit());
                ao_str += "\n";
        }
        // -------------------------------------------------
        //
        ...
        if(a_config.has_cookie_format())
        {
                ...
                ao_str += std::to_string(a_config.cookie_format());
                ao_str += "\n";
        }
        // -------------------------------------------------
        //
        ...
        if(a_config.has_debug_log_level())
        {
                ...
                ao_str += std::to_string(a_config.debug_log_level());
                ao_str += "\n";
        }
        // -------------------------------------------------
        //
                                      ...
                                      const std::string &a_directory)
{
        ...
        {
                int32_t l_status;
                l_status = read_file(ao_config, a_format, *i_file, false);
                if(l_status != WAFLZ_STATUS_OK)
                {
                        // Fail or continue???
                        // Continuing for now...
                                 ...
                                 bool a_force)
{
        ...
        case format_t::MODSECURITY:
        {
                l_status = read_file_modsec(ao_config, a_file, a_force);
                return l_status;
        }
        // -------------------------------------------------
        // JSON
        ...
        case format_t::JSON:
        {
                ...
                l_status = read_file_json(ao_config, a_file, a_force);
                return l_status;
        }
        // -------------------------------------------------
        // PROTOBUF
        ...
        {
                // TODO -share data with engine!!!!
                l_status = read_file_pbuf(ao_config, a_file, a_force);
                return l_status;
        }
        // -------------------------------------------------
        // ???
                                    ...
                                    const std::string &a_path)
{
        ...
        if(l_stat.st_mode & S_IFDIR)
        {
                ...
                if((a_format == format_t::PROTOBUF) ||
                   (a_format == format_t::JSON))
                {
