...
def setup_func():
    ...
    l_subproc = subprocess.Popen([l_waflz_server_path,
                                  ...
                                  '-g', l_geoip2city_path,
                                  '-s', l_geoip2ISP_path])
    #print 'cmd: %s'%([l_waflz_server_path,
    #                              '-g', '/tmp/BOGUS_GEO_DATABASE.db',
    #                              '-f', l_conf_path,
    #                              '-r', l_ruleset_path])
...
def check_rqst(a_host, a_id, a_rqst):
    ...
    print json.dumps(l_r_json)
    if 'response' not in a_rqst:
        assert False, 'no response data in vector to verify: %s'%(json.dumps(a_rqst))
    # ------------------------------------------------------
    # check fields
    # ------------------------------------------------------
...
def test_OWASP_2_2_9_anomaly():
    check_vectors('OWASP_2_2_9.anomaly.msx.vectors.json')
# ------------------------------------------------------------------------------
# owasp 2.2.9 anomaly low inbound score
# ------------------------------------------------------------------------------
def test_OWASP_2_2_9_anomaly_low():
    check_vectors('OWASP_2_2_9.anomaly_low.msx.vectors.json')
# ------------------------------------------------------------------------------
# owasp 3.0.0 anomaly
# ------------------------------------------------------------------------------
def test_ECRS_anomaly():
   check_vectors('ECRS.anomaly.msx.vectors.json')
# ------------------------------------------------------------------------------
# ECRS 3.0.0 anomaly low inbound score
# ------------------------------------------------------------------------------
def test_ECRS_anomaly_low():
   check_vectors('ECRS.anomaly_low.msx.vectors.json')
# ------------------------------------------------------------------------------
# Trustwave_OWASPIntegration_Application anomaly
# ------------------------------------------------------------------------------
#def test_Trustwave_OWASPIntegration_Application_anomaly():
#    check_vectors('Trustwave_OWASPIntegration_Application.anomaly.vectors.json')
